...
# 注意: 新規チャットで PDF だけが添付された状態で送られる self-contained プロンプト。
# 「前回の内容」等の会話継続前提は使えない。
# 自然文応答の癖を抑えるため、extractor の JSON_ONLY_PREFIX/SUFFIX と同等の
# 強い禁止事項を冒頭・末尾の両方に置く。
RETRY_JSON_ONLY_PROMPT = """あなたはデータ抽出専用エンジンです。
...
- 1文字目は必ず { で開始
- 最終文字は }
- 帳票に書かれている項目（ホーム名・利用者名・各種日付・作成者・参加者・計画期間・同意/署名/捺印 等、その帳票に応じて）をできる限り含めること
- 記載のない値は null
