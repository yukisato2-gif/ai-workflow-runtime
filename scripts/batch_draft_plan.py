...
使い方:
    python scripts/batch_draft_plan.py <フォルダパス>
"""
...
from src.tools.sheets_writer import append_draft_plan_result_to_sheet

logger = get_logger(__name__)

DRAFT_PLAN_PROMPT_TEMPLATE = """\
以下のテキストは個別支援計画書案PDFからOCRで抽出したものです。
...
def process_single_pdf(pdf_path: str, claude_client: ClaudeClient) -> bool:
    ...
    try:
        ...
        prompt = DRAFT_PLAN_PROMPT_TEMPLATE.format(ocr_text=text)
        ...
        append_draft_plan_result_to_sheet(pdf_path=pdf_path)
        return True
...
def main() -> None:
    """メイン関数。"""
    if len(sys.argv) < 2:
        print("Usage: python scripts/batch_draft_plan.py <folder_path>")
        sys.exit(1)
